...
def getWordScore(word, n):
    ...
    return score + (sum([SCRABBLE_LETTER_VALUES[char] for char in word]) * len(word))
#
...
def isValidWord(word, hand, wordList):
    ...
    current = {}
    for char in word:
        if current.get(char, 0):
            current[char] += 1
        else:
            current[char] = 1
    return len([char for char in word if char in hand.keys() and hand[char] >= current[char]]) == len(word) and word in wordList
